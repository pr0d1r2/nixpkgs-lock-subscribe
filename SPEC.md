...
| B3 | 2026-08-03 | Updated `flake-manifest` guard rejected helper bindings and an inline `let` in `flake.nix`; strict mode requires outputs delegation | Moved the outputs implementation to `outputs.nix` and reduced `flake.nix` to a thin manifest delegation |
| B4 | 2026-08-06 | `lock-graph-check` found two `nixpkgs` lock nodes because `set-and-setting` resolved its own `nixpkgs-lock` input independently | Made `set-and-setting/nixpkgs-lock` follow the root `nixpkgs-lock` input and regenerated `flake.lock` |
| B5 | 2026-08-18 | Guardrails expected the standard `actions` lefthook fragment, but the fragment list in `flake.nix` and `outputs.nix` omitted it, causing generated `lefthook.yml` fidelity/completeness checks to fail | Added the `actions` fragment to both declarations |
| B6 | 2026-08-18 | `nix flake check` failed while evaluating the actionlint check because the pinned shared helper passed `"^.github/workflows/.*"` as a string to the current Nixpkgs `sourceByRegex` API, which now requires a list | Kept the actionlint gate and defined a repository-local workflow check that invokes `pkgs.actionlint` directly |
| B7 | 2026-08-18 | `linter-coverage-check` required the repository-specific `config/linter-coverage-exemptions.yml`, but the generated repository did not include it | Added the standard linter-coverage exemption policy |
| B8 | 2026-08-18 | `nixfmt-check` rejected the custom `actionlint` check composition in `outputs.nix` because its attribute-set merge was not in canonical Nix formatting | Applied `nixfmt-rfc-style` formatting to `outputs.nix` |
| B9 | 2026-08-18 | `nix flake check` failed while evaluating the standard actionlint check because the shared helper passed a workflow regex string to the current Nixpkgs `sourceByRegex` API, which requires a list | Overrode the consumer `checksFor` helper to retain all standard checks and run `pkgs.actionlint` directly |
